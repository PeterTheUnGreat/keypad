...
void processReceive() {
    ...
    if(chr == MSG_START_CHR) {
        startNewMessage();    // whatever stage we are at, finding a start character initiates the processing of a new message
        return;
    }
    ...
    if((msg_flags & _BV(MSG_DIGIT_ONE)) != 0) {
        temp = getHexChar(chr);
        if(temp < 0) {
            msgState = stateStart;    // if there is an error then put us right back to waiting for the start character
            return;
        }
        msgTemp = temp << 4;
    ...
    case stateAddress:
        ...
        if(msgType == MSG_POLL) msgState = stateEnd;
            ...
            msgState = stateLength;
            msg_flags |= _BV(MSG_DIGIT_ONE);
        }
        break;
    case stateLength:
        if(msgTemp > MAX_MSG_DATA) {
            msgState = stateStart;
            return;
